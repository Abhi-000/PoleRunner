...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (PlayerManager.pm.gameStarted)
        {
            if(!creatingRod)
            ...
            if(canFly)
            {
                ...
                transform.position = MathParabola.Parabola(playerPos, playerPos + Vector3.forward * (PlayerManager.pm.allRods.Count * 2.5f), PlayerManager.pm.allRods.Count / 1.5f, Animation / 2f);
                if (Animation >= 0.5f && !added)
                {
